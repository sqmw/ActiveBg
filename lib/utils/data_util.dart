...
import 'config_util.dart' as config show saveConfig;
import 'package:active_bg/utils/net_util.dart' as net_util show ResponseActions, CommunicationTaskQueueLoop, getUnusedPort, ActiveDynamicBgVideoInfo;
import 'package:active_bg/utils/config_util.dart' as config_util show BgType;
import 'package:active_bg/utils/net_util.dart' as net_util show Data, ResponseActions, ActiveDynamicBgVideoInfo, CommunicationTaskQueueLoop;
...
class DataUtil{
  ...
  /// 这个位置在电脑上面运行之后就不能修改
  static String BASE_PATH = ""; // "lib/assets"
 ...
 //endregion
  ...
  static Future<void> setDynamicBgUrl(String urlOrFilePath)async{
    ...
    if(urlOrFilePath.startsWith("http")){
      /// 这里如果出现错误，检测不到
      tempStr = urlOrFilePath;
    }else{
      /// 启动这个本地壁纸的线程
      int portLocalDynamicBg = await startLocalActiveBg(urlOrFilePath);
      tempStr = "http://localhost:$portLocalDynamicBg?r=${getRandomInt(max: 10000)}";
    }
    /// 设置当前的壁纸的链接或者是文件路径
    net_util.ActiveDynamicBgVideoInfo.urlOrPath = urlOrFilePath;
    ///设置需要返回的数据
    Timer(const Duration(milliseconds: 200),(){
      net_util.CommunicationTaskQueueLoop.addMsg(action: net_util.ResponseActions.changeBg, data: tempStr, doBefore: (){});
    });
    Future.microtask((){
      config.saveConfig();
    });
